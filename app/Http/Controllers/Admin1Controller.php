...
class Admin1Controller extends Controller
{
    ...
    foreach ($year as $key => $value) {
        $user[] = DB::table('dpdcbills')->where('StationId',$value)->value('value');
    }
...
->Value('PV_VALUE');
 ...
 return view('admin.customer_edit',compact('data','config_id'));
    ...
    public function customerupdate(Request $request){
        ...
        $demand_charge = $request->input('demand_charge');
        $previous = $request->input('previous');
        $service = $request->input('service');
        $incremental = $request->input('incremental');
        $vat = $request->input('vat');
        $data=DB::table('stc_stations_1')
        ->where('ID','=',  $config_id)
        ->update(['name'=> $config_value,'address'=>$explanation,'demand_charge'=> $demand_charge,'previous'=>  $previous,'service'=> $service,'incremental'=>        $incremental,'vat'=>$vat]);
