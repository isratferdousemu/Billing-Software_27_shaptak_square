...
<div class="c">    Account Type:General
		...
		<tr class="item-row">
		  ...
		  <td >Consumption</td>
		      ...
		      <td colspan="2"style="text-align:right">Amount(Taka):{{$dept->service }}</td>
		  </tr>
		  ...
		  <tr>
		      ...
		      <td class="total-value"><div id="subtotal">{{ $dept->service }}</div></td>
		  </tr>
...
@if($dept->incremental>0)
<td class="total-value">{{number_format( ($dept->incremental)+(($dept->service)*5)/100,2) }}</td>
@else
...
@if($dept->incremental>0)
<td class="total-value">{{number_format((($dept->service)*5)/100,2)}}</td>
@else
<td class="total-value">0</td>
@endif
		  </tr>
		  ...
		  <tr>
		      ...
		      <td class="total-value balance">  {{number_format(($dept->incremental)+((($dept->service)*5)/100)+($dept->service),2)}}</td>
			  @else
			  <td class="total-value balance">  {{number_format($dept->service,2)}}</td>
			  @endif
		  </tr>
...
@if($dept->incremental>0)
@php
echo"In Word:";echo NumConvert::word( round( ($dept->incremental)+((($dept->service)*5)/100)+($dept->service)));

@endphp
@else
@php
echo"In Word:";echo NumConvert::word( round($dept->service));
