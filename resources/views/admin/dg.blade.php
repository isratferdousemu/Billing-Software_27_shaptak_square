<title>Generator BILL {{$month3}}</title>
<button id="printPageButton" onclick="history.back()" class="text-center"> Back</button>
<button id="printPageButton" onClick="window.print();" class="text-center">Print</button>
...
Zone/Block:{{ $dept->flat}}
     ...
     Name and Address: {{ $dept->name }}</br>
		{{ $dept->address }}
<div class="a">Meter Condition:Normal</br>All Meter consumed unit:{{$unit}}</div><div class="c">    C.T:Ratio:100/5
		...
		<table id="items">
		  ...
		  <td>Common/Check Meter Use:</td>
		      ...
		      <td colspan="2">Amount(Taka):{{number_format($dept->value*$dept->unit,2) }}</td>
		  </tr>
		  ...
		  <tr>
		      ...
		      <td class="total-value"><div id="subtotal">{{number_format($dept->value*$dept->unit,2) }}</div></td>
		  </tr>
		  <tr>
		      <td colspan="2" class="blank"> </td>
		      <td colspan="2" class="total-line">Demand Charge</td>
		      <td class="total-value">{{($dept->demand)}}</td>
		  </tr>
		  <tr>
...
<img  src="images/logo.jpeg" alt="logo" width="150" height="80" />
		      ...
		      <td class="total-value">{{number_format($dept->value*$dept->unit,2) }}</td>
		  </tr>
		  <tr>
		  <td colspan="2" class="blank"> </td>
<td colspan="2" class="total-line">Previous due(With Late Payment Charge)</td>

<td class="total-value">{{ number_format($dept->prev+$dept->late,2)}}</td>
...
<td class="total-value">{{ number_format($dept->late,2)}}</td>
		  ...
		  <tr>
		      ...
		      <td class="total-value balance">{{number_format($dept->total2_col,2) }}</td>
